...
class RunManager:
    ...
    def end_epoch(self):
        ...
        torch.save(self.net, './models_MNIST/best_' + str(self.run.hparams) + '.pth')
    ...
    def save_img(self, grid, filename):
        if not os.path.exists('./gif_MNIST'):
            os.mkdir('./gif_MNIST')
        
        plt.figure(figsize=(15,15))
        plt.imsave('./gif_MNIST/' + filename, np.transpose(grid, (1,2,0)).numpy())
